  ...
  if (beats.length) {
    ...
    function measure() {
      ...
      for (var i = 0; i < nexts.length; i++) {
        ...
        var pinTop = Math.max(selfBox.top, 0);
        var span = vh - pinTop;
        var raw = span > 0 ? (vh - nextTop) / span : 0;
...
(function () {
  ...
  function arm(wrap) {
    ...
    if (!input || !btnClear || !btnOpen || !pop) return;
    ...
    /* A LOCKED FIELD IS PICKED, NEVER TYPED (Fahad, 2026-08-20: "when you
       ...
       nobody can type in and nothing can fill is a dead field. The
       placeholder is set here too, so the markup never promises a picker
       that is not there.
